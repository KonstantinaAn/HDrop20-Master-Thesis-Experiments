...
class Optimization:
    ...
    def __init__(self, worker_dataframe, task_dataframe, k_constrained=None):
        self.worker_df = worker_dataframe
        self.task_df = task_dataframe
        self.k_constrained = k_constrained

    def set_k_constrained(self, k_constrained):
        self.k_constrained = k_constrained
    ...
    def prepare_parameters(self, worker_df=None, task_df=None):
        ...
        # b for k_constrained
        k_array = np.array([self.k_constrained])
    ...
    def wd_ip_solver(self, shape, worker_df, task_df, c=None, c_shape=None, A_ub=None, b_ub=None, verbose=1):
        ...
        if c is None:
            c, A_ub, b_ub, bounds = self.prepare_parameters(worker_df, task_df)
            print('b_ub before', b_ub)
            c_shape = c.shape
            b_ub = np.squeeze(b_ub)
            print('b_ub after', b_ub)
